...
package App::GUI::Sierpingraph::Compute::Image;
use v5.12;
use warnings;
use Benchmark;
...
use App::GUI::Sierpingraph::Compute::Mapping;
...
sub add_progress_bar {
    ...
    $progress_bar{$name} = $bar
        if ref $bar eq 'App::GUI::Sierpingraph::Widget::ProgressBar'
...
sub compute_colors {
    ...
    if ($set->{'mapping'}{'custom_partition'}){
        %gradient_mapping = %{ App::GUI::Sierpingraph::Compute::Mapping::scales(
    ...
    if ($set->{'mapping'}{'use_subgradient'}){
        ...
        my %subgradient_mapping = %{ App::GUI::Sierpingraph::Compute::Mapping::scales(
