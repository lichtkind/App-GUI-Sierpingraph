...
use App::GUI::Sierpingraph::Frame::Part::ColorFlow;
use App::GUI::Sierpingraph::Frame::Part::ColorBrowser;
use App::GUI::Sierpingraph::Frame::Part::ColorPicker;
use App::GUI::Sierpingraph::Frame::Part::PenLine;
use App::GUI::Sierpingraph::Frame::Part::Board;
use App::GUI::Sierpingraph::Dialog::Function;
use App::GUI::Sierpingraph::Dialog::Interface;
use App::GUI::Sierpingraph::Dialog::About;
use App::GUI::Sierpingraph::ProgressBar;
...
sub new {
    ...
    $self->{'color'}{'start'}   = App::GUI::Sierpingraph::Frame::Part::ColorBrowser->new( $self->{'tab'}{'pen'}, 'start', { red => 20, green => 20, blue => 110 } );
    $self->{'color'}{'end'}     = App::GUI::Sierpingraph::Frame::Part::ColorBrowser->new( $self->{'tab'}{'pen'}, 'end',  { red => 110, green => 20, blue => 20 } );

    $self->{'color'}{'startio'} = App::GUI::Sierpingraph::Frame::Part::ColorPicker->new( $self->{'tab'}{'pen'}, $self, 'Start Color IO', $self->{'config'}->get_value('color') , 162, 1);
    $self->{'color'}{'endio'}   = App::GUI::Sierpingraph::Frame::Part::ColorPicker->new( $self->{'tab'}{'pen'}, $self, 'End Color IO', $self->{'config'}->get_value('color') , 162, 7);

    $self->{'color_flow'}       = App::GUI::Sierpingraph::Frame::Part::ColorFlow->new( $self->{'tab'}{'pen'}, $self );
    $self->{'line'}             = App::GUI::Sierpingraph::Frame::Part::PenLine->new( $self->{'tab'}{'pen'} );

    $self->{'progress'}            = App::GUI::Sierpingraph::ProgressBar->new( $self, 450, 5, { red => 20, green => 20, blue => 110 });
    $self->{'board'}               = App::GUI::Sierpingraph::Frame::Part::Board->new( $self , 600, 600 );
    $self->{'dialog'}{'about'}     = App::GUI::Sierpingraph::Dialog::About->new();
    $self->{'dialog'}{'interface'} = App::GUI::Sierpingraph::Dialog::Interface->new();
    $self->{'dialog'}{'function'}  = App::GUI::Sierpingraph::Dialog::Function->new();
    ...
    $help_menu->Append( 13100, "&Function\tAlt+F", "Dialog with information how an Sierpingraph works" );
...
sub open_settings_dialog {
    ...
    if (not ref $ret) { $self->SetStatusText( $ret, 0) }
        ...
        my $dir = App::GUI::Sierpingraph::Settings::extract_dir( $path );
...
sub save_image_dialog {
    ...
    else     { $self->{'config'}->set_value('save_dir', App::GUI::Sierpingraph::Settings::extract_dir( $path )) }
}
...
sub inc_base_counter {
    ...
    $dir = App::GUI::Sierpingraph::Settings::expand_path( $dir );
...
sub change_base_dir {
    ...
    $new_dir = App::GUI::Sierpingraph::Settings::shrink_path( $new_dir ) . '/';
...
sub open_setting_file {
    my ($self, $file ) = @_;
    my $data = App::GUI::Sierpingraph::Settings::load( $file );
    if (ref $data) {
        $self->set_data( $data );
        $self->draw;
